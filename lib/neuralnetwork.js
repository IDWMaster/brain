...
NeuralNetwork.prototype = {
   ...
   */
  trainFunction:function(input,targetFunction,learningRate) {
      learningRate = learningRate || this.learningRate;
    ...
    // back propogate
    this.calculateDeltasForFunction(targetFunction);
    this.adjustWeights(learningRate);
  ...
  calculateDeltasForFunction: function(target) {
      target = target(this.outputs[this.outputLayer]);
    for (var layer = this.outputLayer; layer >= 0; layer--) {
      for (var node = 0; node < this.sizes[layer]; node++) {
        var output = this.outputs[layer][node];
